        ...
        map.put(policy, attributesArray);
         ...
         output.close();
                            ...
                            try {
                                ...
                                pstm = con.prepareStatement("insert into uploadcloud (filename, data, owner,skey,policy,time,exp,branch)values(?,?,?,?,?,?,?,?)");
                               ...
                               Ftpcon ftpcon = new Ftpcon();
                                ...
                                int i = pstm.executeUpdate();
                                 ...
                                 PreparedStatement ps=con.prepareStatement("update uploadcloud set status='Uploaded' where filename='" + filename + "' ");
	                         ps.executeUpdate();
                                 
                                 PreparedStatement pst=con.prepareStatement("update upload set status='Uploaded' where filename='" + filename + "' ");
	                         pst.executeUpdate();
                                 
                                if (i == 1) {
                              response.sendRedirect("cloudviewfiles.jsp?upload=File Upload Success");
                                } else {
                                    
                                    response.sendRedirect("cloudviewfiles.jsp?upload1=File Upload Failed");
                                }
                               
                            } catch (Exception e) {
                                
                                 PreparedStatement pst=con.prepareStatement("update upload set status='Duplicate' where filename='" + filename + "' ");
	                         pst.executeUpdate();
                                 response.sendRedirect("cloudviewfiles.jsp?upload2=File Already Uploaded");
                    ...
                    Logger.getLogger(uploadcloud.class.getName()).log(Level.SEVERE, null, ex);
                }
            } finally {
                out.close();
            }
